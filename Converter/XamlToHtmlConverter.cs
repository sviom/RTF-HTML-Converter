...
namespace RichTextBoxResearch
{
    ...
    {
        private static XmlDocument DocReader { get; set; } = new XmlDocument();
        private static XmlDocument DocWriter { get; set; } = new XmlDocument();
        ...
        public static string ConvertXamlToHtml(string xamlString, bool asFullDocument)
        {
            ...
            DocReader.LoadXml(xamlString);


            //XmlElement dddd = Doc.CreateElement("", "", "");
        ...
        /// </param>
        private static bool WriteFlowDocument(XmlTextReader xamlReader, XmlTextWriter htmlWriter, bool asFullDocument = false)
        {
        ...
        /// </param>
        private static void WriteFormattingProperties(StringBuilder inlineStyle)
        {
            //Debug.Assert(xamlReader.NodeType == XmlNodeType.Element);
            //Debug.Assert(Doc.NodeType == XmlNodeType.Element);
            ...
            foreach (XmlNode xmlItem in DocReader.ChildNodes)
            {
                string css = null;
                switch (xmlItem.Name)
                {
                    ...
                    // ------------------------------
                    case "Background":
                        css = "background-color:" + ParseXamlColor(xmlItem.Value) + ";";
                        break;
                    case "FontFamily":
                        css = "font-family:" + xmlItem.Value + ";";
                        break;
                    case "FontStyle":
                        css = "font-style:" + xmlItem.Value.ToLower() + ";";
                        break;
                    case "FontWeight":
                        css = "font-weight:" + xmlItem.Value.ToLower() + ";";
                        break;
                    case "FontStretch":
                        break;
                    case "FontSize":
                        css = "font-size:" + xmlItem.Value + ";";
                        break;
                    case "Foreground":
                        css = "color:" + ParseXamlColor(xmlItem.Value) + ";";
                        break;
                    case "TextDecorations":
                        if (xmlItem.Value.ToLower() == "strikethrough")
                    ...
                    // -------------------------------
                    case "Padding":
                        css = "padding:" + ParseXamlThickness(xmlItem.Value) + ";";
                        break;
                    case "Margin":
                        css = "margin:" + ParseXamlThickness(xmlItem.Value) + ";";
                        break;
                    case "BorderThickness":
                        css = "border-width:" + ParseXamlThickness(xmlItem.Value) + ";";
                        borderSet = true;
                        break;
                    case "BorderBrush":
                        css = "border-color:" + ParseXamlColor(xmlItem.Value) + ";";
                        borderSet = true;
                        break;
                    case "LineHeight":
                        break;
                    case "TextIndent":
                        css = "text-indent:" + xmlItem.Value + ";";
                        break;
                    case "TextAlignment":
                        css = "text-align:" + xmlItem.Value + ";";
                        break;
                    case "IsKeptTogether":
                        break;
                    case "IsKeptWithNext":
                    ...
                    // ----------------
                    case "Width":
                        css = "width:" + xmlItem.Value + ";";
                        break;
                    case "ColumnSpan":
                        //htmlWriter.WriteAttributeString("COLSPAN", xmlItem.Value);
                        DocWriter.CreateAttribute("COLSPAN", xmlItem.Value);
                        break;
                    case "RowSpan":
                        //htmlWriter.WriteAttributeString("ROWSPAN", xmlItem.Value);
                        DocWriter.CreateAttribute("ROWSPAN", xmlItem.Value);
                        break;

                    // Hyperlink Attributes
                    case "NavigateUri":
                        //htmlWriter.WriteAttributeString("HREF", xmlItem.Value);
                        DocWriter.CreateAttribute("HREF", xmlItem.Value);
                        break;

                    case "TargetName":
                        //htmlWriter.WriteAttributeString("TARGET", xmlItem.Value);
                        DocWriter.CreateAttribute("TARGET", xmlItem.Value);
                        break;
                    default:
                        break;
                }
        ...
        /// DocReader의 Xaml Element를 읽고 그것을 변환한다.
        /// </summary>
        /// <param name="xamlReader">
        /// XmlTextReader which is expected to be at XmlNodeType.Element
        /// (opening element tag) position.
        ...
        private static void WriteElementContent(XmlTextReader xamlReader, XmlTextWriter htmlWriter, StringBuilder inlineStyle)
        {
            ...
            if (DocReader.HasChildNodes)
            {
                if (DocWriter != null && !elementContentStarted && inlineStyle.Length > 0)
                {
