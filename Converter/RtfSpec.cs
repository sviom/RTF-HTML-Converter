...
namespace RichTextBoxResearch.Converter
{
    ...
    public static class RtfSpec
    {
        ...
        public static string GetHtmlFromRtfCode(string rtfCode, string valueText = "")
        {
            string returnValue = string.Empty;
            switch (rtfCode)
            {
                case TagBold:
                    returnValue = "<b>" + valueText;// + "</b>";
                    break;
                case TagBold + "0":
                    returnValue = "</b>";
                    break;
                case TagParagraph:
                case TagParagraphDefaults:
                    returnValue = valueText + "<br />";
                    break;
                case TagUnderLine:
                    //if (!string.IsNullOrEmpty(valueText))
                    returnValue = "<span style='text-decoration: underline;'>";// + valueText + "</span>";
                    break;
                case TagUnderLineNone:
                    returnValue = "</span>";
                    break;
                default:
                    if (!string.IsNullOrEmpty(valueText))
